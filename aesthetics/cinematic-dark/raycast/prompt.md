# 电影暗调 · Raycast 设计系统
> SDesign 去 AI 味设计系统 · 美学来源 open-design(`raycast`,Apache-2.0)
> **用法**:把下面 ```text 代码块里的整段**复制**,粘给你的 AI(Claude Code / ChatGPT / Codex / 任意),让它做**网页 / 落地页**(本套 token 是 CSS 单位)。

```text
# 美学:电影暗调(Cinematic Dark)— Raycast 风格
电影级渐变 + 超大字 + 深色 + 媒体重 hero;震撼、高级、创作工具气质。

# 设计 token(必须用这些精确值,不准自创别的颜色)
- 背景:#07080a
- 表面/卡片:#101111 ,边框 rgba(255, 255, 255, 0.06)
- 正文主色:#f9f9f9 ;次级:#cecece ;弱化:#9c9c9d
- 强调色(只此一个):#FF6363 —— 只用在唯一焦点 / 主 CTA / 小标签
- 圆角:卡片 16px,按钮 12px
- 标题字体:"Inter", "Inter Fallback", "SF Pro Display", -apple-system, system-ui, "Segoe UI", Roboto, sans-serif
- 正文字体:"Inter", "Inter Fallback", "SF Pro Text", -apple-system, system-ui, "Segoe UI", Roboto, sans-serif
- 行高:正文 1.6,标题 1.10
- ⚠️ 字体若为私有/不可公开(Anthropic Serif、SF Pro、商业 Inter Variable 等),换 Google Fonts 最接近的公开字体并真正加载,保证浏览器能渲染。

# 本美学特别说明(凡与通用规则冲突,以此为准)
可用大尺度渐变、光影、超大字、媒体重 hero 做震撼;深色电影感、动效优先。 玻璃卡技法:真毛玻璃 `backdrop-filter:blur(20px)`(浮起层 40px)+ 半透白填充 `rgba(255,255,255,.08~.12)` + 1px 白边 `rgba(255,255,255,.18)` + 内高光只走顶+左两条边(单光源)+ 柔投影 `0 8px 32px rgba(0,0,0,.3)`;玻璃上字重加一档、小标签加 text-shadow 保可读。
