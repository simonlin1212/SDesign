# 暖色人文 · Mistral AI 设计系统
> SDesign 去 AI 味设计系统 · 美学来源 open-design(`mistral-ai`,Apache-2.0)
> **用法**:把下面 ```text 代码块里的整段**复制**,粘给你的 AI(Claude Code / ChatGPT / Codex / 任意),让它做**网页 / 落地页**(本套 token 是 CSS 单位)。

```text
# 美学:暖色人文(Warm Humanist)— Mistral AI 风格
奶油/陶土暖色 + 衬线大标题 + 大量留白;沉静、有人味、手作感。

# 设计 token(必须用这些精确值,不准自创别的颜色)
- 背景:#fffaeb
- 表面/卡片:#fff0c2 ,边框 #f0e8c8
- 正文主色:#1f1f1f ;次级:var(--fg) ;弱化:#3d3d3d
- 强调色(只此一个):#fa520f —— 只用在唯一焦点 / 主 CTA / 小标签
- 圆角:卡片 4px,按钮 2px
- 标题字体:Arial, ui-sans-serif, system-ui, sans-serif
- 正文字体:Arial, ui-sans-serif, system-ui, sans-serif
- 行高:正文 1.5,标题 1.10
